...
DOCKER_ID=$(sudo docker run --rm --name="$tag" -div --cpus="$cpus" --memory="$memory" $(pwd):/openLANE_flow -v $PDK_ROOT:$PDK_ROOT -e PDK_ROOT=$PDK_ROOT -u $(id -u $USER):$(id -g $USER) openlane:cloud)
